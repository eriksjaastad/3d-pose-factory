...
You have access to an SSH Agent that reads commands from `ops_queue/requests.jsonl`
and writes results to `ops_queue/results.jsonl`.

## How to request a command

- Append **one JSON object per line** to `ops_queue/requests.jsonl`.
...
- The agent appends JSON lines to `ops_queue/results.jsonl` with this shape:
...
- When you need the output of a command you requested, scan `ops_queue/results.jsonl`
